...
- **Sub-resource requests that reach the proxy root are attributed by `Referer`, never by ambient state.** Some Grafana sub-resource requests escape the `appSubUrl`/`<base>` rewrite and arrive with neither the token nor the instance in their own path. These are resolved from the URL of the document that issued them, checked same-origin and re-validated through the same token and instance-id parsers as a normal route; responses carry `Referrer-Policy: same-origin` so that URL is available. A request that still cannot be attributed gets the bare 404 rather than being routed by guess.

  Until 2026-08-13 this fallback used two `Path=/` cookies instead. A cookie is ambient — one slot per origin, shared by every instance framed against the proxy — so with two dashboards open, the one that loaded last owned the slot and the other's bare requests were routed to the wrong instance **and signed with that instance's token**. Scoping the cookie to `Path=/instances/<id>` does not fix it: a cookie path only matches request paths beneath it, so it would never be sent on the root-relative requests the fallback exists to serve. The referrer is per-request, so N instances stay independent.
