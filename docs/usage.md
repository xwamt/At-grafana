...
Once enabled, all 17 MCP tools become usable against that instance's `instanceId` at any time — the Agent does not need any dashboard/alert Webview panel open first. Disabling the toggle immediately blocks all further tool calls against that instance, and it stops appearing in `grafana_list_instances`.
...
3. Reload/reconnect your MCP client if it doesn't pick up the new config automatically. All 17 `grafana_*` tools should now be listed and pre-approved (no manual per-tool approval needed, since every tool is `risk: read`).
...
The catalog is discovery (`grafana_list_instances`), eight management tools, and eight monitoring tools. `grafana_list_dashboards` accepts optional `query` / `tag` / `folderUid`. `grafana_get_dashboard` defaults to `fields: "targets"` (complete model requires `fields: "full"`). `grafana_list_alert_rules` accepts optional `states`. `grafana_generate_deeplink` always returns `grafanaUrl` (`openInIde` default false). Prefer `grafana_query_prometheus` / `grafana_query_loki` after the four Prom/Loki list tools; keep `grafana_query_datasource` as a `GET`/`POST` escape hatch with path confinement and `truncated: true` on over-cap responses.
